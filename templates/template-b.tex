...
% 章节标题居中，二级标题左对齐
\titleformat{\section}{\centering\zihao{2}\bfseries\heiti}{\thesection}{0.8em}{}
\titleformat{\subsection}{\zihao{-3}\bfseries\heiti}{\thesubsection}{0.8em}{}
\titleformat{\subsubsection}{\zihao{4}\bfseries\heiti}{\thesubsubsection}{0.6em}{}

% 页眉空、页脚页码居中
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small 2026\,\text{高教社杯全国大学生数学建模竞赛}\quad\texttt{XX题}\quad|\quad 队伍\texttt{XXXX}}
...
\paragraph{消融对照(基础 vs 创新)}
% 国一标准：每问必须给「基础方案 vs 创新方案」双方案对照，量化超越幅度。
% 数值来自 results/ablation.csv，禁止凭空编造。
\begin{table}[H]
\centering
\caption{消融对照：基础方案 vs 创新方案}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
指标 & 基础方案(均匀布局) & 创新方案(SA-PSO) & 提升 \\
\midrule
年均功率 & 45.3 MW & 54.2 MW & +19.6\% \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../figures/pdf/figN_residual_qq.pdf}
\caption{图N 拟合残差直方图与 Q-Q 图（残差近似正态分布）}
\label{fig:residual}
\end{figure}

\subsection{灵敏度分析}
% Sobol 全局灵敏度优先（一阶 S1 + 总阶 ST + 95%CI），单参数龙卷风图为补充
\begin{table}[H]
\centering
\caption{Sobol 全局灵敏度（S1 一阶 + ST 总阶）}
\label{tab:sobol}
\begin{tabular}{lcc}
\toprule
参数 & S1 & ST \\
...
% MC 结果直方图 + 累计分布函数 CDF 图
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../figures/pdf/figN_mc_cdf.pdf}
\caption{图N 蒙特卡洛结果直方图与累计分布函数（CDF）}
\label{fig:mc}
\end{figure}
...
\begin{table}[H]
\centering
\caption{假设误差量化（每项含风险等级：致命/显著/轻微/可忽略）}
\label{tab:assumption_error}
\begin{tabular}{lccl}
\toprule
假设 & 偏差量化 & 对结果影响 & 风险等级 \\
\midrule
假设1:镜面理想平面 & $\delta = 0.5\%$ & 功率损失 $1.2\%$ & 轻微 \\
假设2:无大气衰减 & $\delta = 2.1\%$ & 功率损失 $3.5\%$ & 显著 \\
假设3:稳态光照 & $\delta = 1.8\%$ & 功率波动 $\pm 2.0\%$ & 轻微 \\
\bottomrule
\end{tabular}
\end{table}

% ============ 跨问题对照表（国一标准：横向对比 + 变化率列） ============
\subsection{跨问题结果对照}
\begin{table}[H]
\centering
\caption{各问结果横向对照（含相对变化率）}
\label{tab:cross_compare}
\begin{tabular}{lccc}
\toprule
问题 & 核心指标 & 基础方案 & 本文方案(变化率) \\
\midrule
问题一 & 效率诊断 & 均匀布局 & 多尺度拆解(识别错配) \\
问题二 & 布局优化 & 传统布局 & SA-PSO(+19.6\%) \\
问题三 & 参数寻优 & 单目标 & 多目标帕累托(+28\%) \\
\bottomrule
\end{tabular}
\end{table}
...
% \lstinputlisting[language=Python, caption=code/02_q2_classify.py]{../code/02_q2_classify.py}
% \lstinputlisting[language=Python, caption=code/03_q3_optimize.py]{../code/03_q3_optimize.py}
% \lstinputlisting[language=Python, caption=code/04_q4_robust.py]{../code/04_q4_robust.py}
% \lstinputlisting[language=Python, caption=code/05_figures.py]{../code/05_figures.py}

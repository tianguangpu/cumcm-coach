...
\section{附录2 代码}
\begin{lstlisting}[language=Python, caption=problem1_predict.py]
import numpy as np
from arima import ARIMA_Forecast
from mlp import MLP_Forecast

# ARIMA 线性成分（统计模型，numpy+scipy 自研实现）
arima_model = ARIMA_Forecast(y_train, order=(3, 1, 2)).fit()
arima_pred = arima_model.predict(steps=3)

# MLP 非线性成分（sklearn 实现，无 torch 依赖；如需真 LSTM 可 pip install torch 后替换）
mlp_model = MLP_Forecast(y_train, window=12, hidden_layer_sizes=(50,)).fit()
mlp_pred = mlp_model.predict(steps=3)

# 组合预测（ARIMA + MLP 加权融合）
final_pred = 0.5 * arima_pred + 0.5 * mlp_pred
\end{lstlisting}

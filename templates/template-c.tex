...
\section{附录2 代码}
\begin{lstlisting}[language=Python, caption=problem1_evaluate.py]
from ahp_entropy_topsis import ComprehensiveEvaluation

ce = ComprehensiveEvaluation(
    data=X,                  # 原始数据矩阵 (n_samples, n_features)
    benefit_cols=[0, 1, 2],  # 效益型指标索引
    cost_cols=[3, 4],        # 成本型指标索引
    col_names=['效率', '质量', '稳定性', '可维护性', '成本'],
)
ce.run_ahp(A)              # AHP 判断矩阵 → 主观权重
ce.run_entropy()           # 熵权法 → 客观权重
ce.combine_weights(alpha=0.5)  # 组合赋权
scores = ce.topsis()       # TOPSIS 排序 → 综合得分向量
\end{lstlisting}

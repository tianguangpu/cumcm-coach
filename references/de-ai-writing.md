...
✅ 模型在长尾类别上的提升尤为明显（+12.3%），因为 Focal Loss 降低了易分样本的权重。 （有数字 + 有因果解释）
```
...
## 六、AIGC 检测专项（2024年起新增）

> 2024年起国赛引入 AIGC 检测机制。本节提供针对 AIGC 检测的专项改写策略。
> 详细指南见 `references/aigc-awareness.md`。

### AIGC 检测的核心指标

| 指标 | AI 特征 | 人类特征 | 改写策略 |
|------|---------|---------|---------|
| **困惑度** | 用词"安全"、高频 | 用词多样、偶尔生僻 | 替换同义词为低频词 |
| **突发性** | 句子长度均匀 | 长短交替 | 故意制造长短句交替 |
| **词汇多样性** | 重复使用高频词 | 用词丰富 | 每段避免重复用词 |
| **逻辑连贯性** | "太完美"的逻辑 | 有自然跳跃 | 适当加入转折和跳跃 |
...
1. **公式替代文字**：LaTeX 公式不参与 AIGC 检测，公式越多 AIGC 比例越低
2. **数据密集写作**：每段嵌入 ≥1 个具体数值
3. **首尾句人工写**：AIGC 检测对首尾句更敏感
4. **打乱句子结构**：将 AI 的"总-分"结构改为"分-总"
5. **添加实验细节**：参数设置、数据来源等细节
...
本文档已被集成到 `cumcm-coach-skill-v7` 的 `references/` 目录。

**调用时机**：
1. **阶段 3 论文撰写** — 写完初稿后，用本文档逐条自查
2. **阶段 4 L1-L4 评审** — L3 对抗评审时，用本文档检查 AI 味
3. **阶段 4 L4 终审** — 最终交付前，用快速自检清单过一遍

**配套脚本**：
- `scripts/polish_abstract.py` — 摘要 AI 味检测（已有）
- `scripts/writing_check.py` — 写作质量检查（已有）
- 本文档 — 人工自查指南（新增）

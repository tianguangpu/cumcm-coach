...
def export_figure(
    ...
    Args:
        ...
        journal: 期刊名（nature/science/ieee/aaas/aps/default），自动应用
            SciencePlots + tueplots 的配色/字号/尺寸规范。需 v7_styles.py。
    Returns:
        实际写出的文件路径列表。
    """
    if formats is None:
    ...
    # 应用期刊风格（SciencePlots + tueplots）
    if journal is not None:
        try:
            from v7_styles import apply_journal_style
            apply_journal_style(journal)
        except ImportError:
            print("[scipilot-figure-skill] v7_styles not found; "
                  "journal style skipped.", file=sys.stderr)
    # 强制嵌入 TrueType 字体（fonttype 42）

...
metadata:
  version: "2.1.1"
  updated: "2026-08-05"
  changelog: "绘图分工边界（国赛选型走 diagram-master、国赛 Python 走 figure-skill）"
---
...
## 与其他绘图 Skill 的分工边界（避免触发冲突）

| Skill | 定位 | 触发场景 |
|---|---|---|
| `math-modeling-diagram-master` | 国赛图表**选型决策**（题型→fig_plan 清单）+ MATLAB `academic_plotlib` 模板分发 | 国赛/CUMCM 题目出图第一步 |
| `figure-skill` | 国赛 Python **绘图规范**（SimHei/5 套调色板/finish_figure 双导出/图题含结论） | 国赛论文 Python 出图执行 |
| **本 Skill** | 科研出版级可视化顾问（EDA→选图→期刊规范→视觉自检闭环） | 期刊投稿/毕业论文/通用科研数据图 |

国赛论文图表优先走 diagram-master + figure-skill；本 Skill 在用户提到期刊投稿、数据剖析、或"图达不到出版要求"时激活。两者可衔接：国赛结果图也可用本 Skill 的视觉自检闭环（visual_qa + AI 读图）做终检。

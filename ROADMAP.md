...
当前 **52%**（algorithms/ 实测，2026-09-13），缺口集中在几个大模块：
...
**测试薄 = 用户"看起来能用、一跑就错"的风险，欢迎贡献单元测试。**

**覆盖率分级（哪些关键、哪些次要）**：

| 区域 | 关键度 | 目标覆盖率 | 现状 |
|------|--------|-----------|------|
| `scripts/auto_check.py`（L1 检查）| 🔴 关键 | ≥80% | 偏低（未单独统计）|
| 算法核心（GA / NSGA2 / AHP+TOPSIS / Sobol）| 🔴 关键 | ≥70% | 仅接口冒烟 |
| `scripts/run_all.py`（流水线）| 🟡 中 | ≥60% | 未单独统计 |
| 文档 / 示例 | 🟢 次要 | 不计入 | — |

> 52% 是 algorithms/ 的**平均值**，掩盖了关键路径覆盖薄的问题。优先把
> L1 检查脚本和算法核心补到目标线，再谈整体 80%。
...
**哪些接受 PR、哪些核心不动**（降低贡献门槛，欢迎社区参与）：

| 区域 | 接受 PR | 说明 |
|------|---------|------|
| `algorithms/` 单元测试 | ✅ 强烈欢迎 | 测试覆盖率 52% 偏低，补测试是最高价值贡献 |
| `examples/` 新示例 | ✅ 欢迎 | 题型×算法示例矩阵，多多益善 |
| `docs/` 文档 | ✅ 欢迎 | 新手路径、迭代记录、错别字 |
| `templates/` 模板 | ⚠️ 谨慎 | 需保持 4 题型一致性 + 编译通过 |
| `scripts/auto_check.py` 检查项 | ⚠️ 谨慎 | 核心门禁，改逻辑需加测试 |
| 题型识别核心规则 | ❌ 核心不动 | 见 `SKILL.md` §1.1，改动需充分讨论 |
